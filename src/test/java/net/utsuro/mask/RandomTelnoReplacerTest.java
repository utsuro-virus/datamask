...
class RandomTelnoReplacerTest extends RandomTelnoReplacer {
  ...
  class Execute {
    ...
    void case4() throws Exception {
      ...
      assertTrue(ret.matches("^0[0-9]{10}"));
    }
    ...
    @Test
    @DisplayName("ランダム置換の電話番号が返る")
    void case10() throws Exception {
      String ret;
      ret = replace("090-1234-5678", rule);
      assertTrue(ret.matches("^0[0-9]{2}-[0-9]{4}-[0-9]{4}"));
      ret = replace("(043)234-678", rule);
      assertTrue(ret.matches("^\\(0[0-9]{2}\\)[0-9]{3}-[0-9]{3}"));
      ret = replace("123456789", rule);
      assertTrue(ret.matches("^0[0-9]{8}"));
    }
    ...
    void case90() throws Exception {
      ...
      assertTrue(((String) execute("(043)234-678", rule)).matches("^\\(0[0-9]{2}\\)[0-9]{3}-[0-9]{3}"));
    }
  ...
  class ExecuteWithDatabase {
    ...
    void case2() throws Exception {
      ...
      DataMask converter = spy(new RandomTelnoReplacer());
      ...
      for (int i = 0; i < count; i++) {
        // ユニークなので一通り存在するはず
        assertTrue(retList.indexOf(String.format("%02d", i)) >= 0);
        // 1回は必ず登場する
        verify(converter, atLeast(1)).isExistsInUniqueList("HOGE", String.format("%02d", i));
        // 23回以上(50%)同じ値で呼ばれていたら偏りすぎ
        verify(converter, atMost(22)).isExistsInUniqueList("HOGE", String.format("%02d", i));
      }
    }
